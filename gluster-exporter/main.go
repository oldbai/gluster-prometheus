...
func main() {
	...
	if err != nil {
		...
		if exporterConf.Auth.User != "" && exporterConf.Auth.Secret != "" {
			http.Handle(metricsPath, authMiddleware(promhttp.Handler(), exporterConf.Auth.User, exporterConf.Auth.Secret))
		} else {
...
// authMiddleware is a middleware to authenticate the requests
func authMiddleware(handler http.Handler, user string, secret string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("X-Gluster-Exporter-Auth-User") != user || r.Header.Get("X-Gluster-Exporter-Auth-Secret") != secret {
